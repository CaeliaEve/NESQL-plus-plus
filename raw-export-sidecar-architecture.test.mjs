...
const finalReportCatalogUrl = new URL(
  './src/main/java/com/github/dcysteine/nesql/exporter/local/RawExportFinalReportCatalog.java',
  import.meta.url,
);
...
test('raw export semantic runtime report factory and file ops are split from sidecar', () => {
  ...
  assert.equal(existsSync(finalReportCatalogUrl), true, 'RawExportFinalReportCatalog must exist');
  ...
  assert.match(finalReportCatalog, /FINAL_REPORT_COPIES = validateAndFreeze\(Arrays\.asList\(/);
  assert.match(finalReportCatalog, /RawExportFileCatalog\.validationPath\(RawExportFileCatalog\.STAGE_CHECKSUMS_FILE_NAME\)/);
  assert.match(finalReportCatalog, /ExportDebugFile\.STAGE_TIMING\.validationAliasPath\(\)/);
