...
test('native NEI handler layouts export GT dynamic primitives and background regions at the source', () => {
  ...
  assert.equal(nativeUiAbi.includes('GT_NEI_BACKGROUND_ASSET_REF'), false);
  assert.equal(nativeUiAbi.includes('GT_NEI_BACKGROUND_RESOURCE'), false);
  ...
  assert.equal(rawNeiFactWriter.includes('materializeGtNeiBackgroundAsset(rawDir)'), false);
  ...
  assert.equal(rawNeiFactWriter.includes('GTNEIDefaultHandler slot geometry reference'), true);
  ...
  assert.equal(rawNeiFactWriter.includes('Failed to materialize required GT NEI ModularUI background asset'), false);
  assert.equal(rawNeiFactWriter.includes('Logger.MOD.warn("Could not materialize GT NEI ModularUI background asset'), false);
  assert.equal(rawNeiFactWriter.includes('StandardCopyOption.ATOMIC_MOVE'), false);
  assert.equal(nativeUiAbi.includes('UI_BACKGROUNDS_DIRECTORY'), false);
  assert.equal(executionStrategy.includes('NativeNeiFrameExportRegistry'), false);
  ...
  assert.equal(templateWriter.includes('template.nativeBackground.assetRef'), false);
  ...
  assert.equal(nativeUiEvidenceCatalog.includes('RAW_COUNT_DESCRIPTORS'), true);
  assert.equal(nativeUiEvidenceCatalog.includes('NATIVE_UI_LAYOUTS'), true);
  assert.equal(nativeUiEvidenceCatalog.includes('NATIVE_UI_INTERACTION_CONTRACT_VIOLATIONS'), true);
  assert.equal(nativeUiEvidenceCatalog.includes('populateRawCounts'), true);
