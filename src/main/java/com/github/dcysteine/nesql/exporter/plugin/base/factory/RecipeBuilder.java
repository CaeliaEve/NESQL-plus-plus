...
public class RecipeBuilder extends PluginHelper {
    ...
    public RecipeBuilder addItemInputAt(int slotIndex, net.minecraft.item.ItemStack input) {
        if (slotIndex < 0) {
            return addItemInput(input);
        }
        if (input == null) {
            return skipItemInputAt(slotIndex);
        }
        itemInputs.put(slotIndex, itemGroupFactory.get(input));
        itemInputsIndex = Math.max(itemInputsIndex, slotIndex + 1);
        return this;
    }
    ...
    public RecipeBuilder addItemGroupInputAt(
            int slotIndex, Collection<net.minecraft.item.ItemStack> inputs) {
        if (slotIndex < 0) {
            return addItemGroupInput(inputs);
        }
        if (inputs == null || inputs.stream().allMatch(Objects::isNull)) {
            return skipItemInputAt(slotIndex);
        }
        itemInputs.put(slotIndex, itemGroupFactory.get(inputs));
        itemInputsIndex = Math.max(itemInputsIndex, slotIndex + 1);
        return this;
    }
    ...
    public RecipeBuilder addItemGroupInputAt(int slotIndex, net.minecraft.item.ItemStack[] inputs) {
        return addItemGroupInputAt(slotIndex, Arrays.asList(inputs));
    }
    ...
    public RecipeBuilder skipItemInputAt(int slotIndex) {
        if (slotIndex < 0) {
            return skipItemInput();
        }
        itemInputsIndex = Math.max(itemInputsIndex, slotIndex + 1);
