...
/** Command to rebuild render canonical/atlas/index outputs from an existing NESQL++ repository. */
final class RenderContractsExportCommand implements ICommand {
    @Override
    public String getCommandName() {
        return "nesql-render-contracts";
    }

    @Override
    public String getCommandUsage(ICommandSender unused) {
        return "/nesql-render-contracts [repository suffix]";
    }
    ...
    public void processCommand(ICommandSender sender, String[] args) {
        ...
        RenderContractsExporter exporter =
                args.length == 1 ? new RenderContractsExporter(args[0]) : new RenderContractsExporter();

        ExportProgressGui gui = new ExportProgressGui();
        gui.clear();
        gui.setTitle("NESQL++ 1.04");
        gui.setSubtitle("Render Contract Rebuild / " + (args.length == 1 ? args[0] : "default"));
        gui.addMessage("Rebuilding render-assets / animation / atlas / render-index...");
        gui.addMessage("Reuses existing data + rendered images only.");
        gui.addMessage("Export target: canonical/render-assets.json");
