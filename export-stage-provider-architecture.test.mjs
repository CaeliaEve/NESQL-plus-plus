...
test('export control and debug planes have explicit filesystem ownership', () => {
  ...
  assert.match(debugPlaneWriter, /new DebugReportDescriptor\(\s*ExportDebugFile\.STAGE_CHECKPOINT/);
  assert.match(debugPlaneWriter, /new DebugReportDescriptor\(\s*ExportDebugFile\.KERNEL_TRACE/);
  assert.match(debugPlaneWriter, /writeDebugReport\(\s*exportContext,\s*ExportDebugFile\.STAGE_TIMING/);
  assert.match(debugPlaneWriter, /writeDebugReport\(\s*exportContext,\s*ExportDebugFile\.STAGE_CHECKPOINT/);
  assert.match(debugPlaneWriter, /writeDebugReport\(\s*exportContext,\s*ExportDebugFile\.KERNEL_TRACE/);
  ...
  assert.match(debugPlaneWriter, /private interface DebugReportFactory/);
  assert.match(debugPlaneWriter, /private static final class DebugReportDescriptor/);
...
test('export validation probe catalog owns report collection order and capabilities', () => {
  ...
  assert.match(validationEvidenceCatalog, /RAW_ITEMS = descriptorValue\(DESCRIPTORS, "rawItems"\)/);
  assert.match(validationEvidenceCatalog, /TOP_UNCLASSIFIED_FAMILY_ACTIONS =\s*\r?\n?\s*descriptorValue\(DESCRIPTORS, "topUnclassifiedFamilyActions"\)/);
  assert.match(validationEvidenceCatalog, /PRIMARY_ARTIFACT = descriptorValue\(DESCRIPTORS, "primaryArtifact"\)/);
  assert.match(validationEvidenceCatalog, /ITEM_COUNT = descriptorValue\(DESCRIPTORS, "itemCount"\)/);
  assert.match(validationEvidenceCatalog, /HANDLERS_WITH_LOADED_RECIPES =\s*\r?\n?\s*descriptorValue\(DESCRIPTORS, "handlersWithLoadedRecipes"\)/);
  assert.match(validationEvidenceCatalog, /ERROR_FIELD_SCHEMA_VERSION = descriptorValue\(ERROR_FIELD_DESCRIPTORS, "schemaVersion"\)/);
